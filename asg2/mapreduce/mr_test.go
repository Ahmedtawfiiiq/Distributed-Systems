...
func makeInputs(num int) string {
	...
	for f := 0; f < num; f++ {
		...
		file, err := os.Create(filepath.Join(indir, names[f]))
		if err != nil {
			log.Fatal("mkInput: ", err)
		}
...
// Cook up a unique-ish UNIX-domain socket name in /var/tmp.
// The code has been adapted to work on windows as well
func port(suffix string) string {
	...
	if runtime.GOOS == "windows" {
		...
		err = os.Mkdir(filepath.Join(dir, "tmp552"), 0755)
		if err != nil && !os.IsExist(err) {
			log.Fatal("port: ", err)
		}
		s = filepath.Join(dir, "tmp552\\552")
	}
	s += filepath.Join(strconv.Itoa(os.Getuid()), "")
	os.Mkdir(s, 0777)
	s += "mr"
...
func TestBasic2(t *testing.T) {
	mr := setup()
	// creating many worker threads (more than what we need)
	for i := 0; i < nReduce+nMap+10; i++ {
		go RunWorker(mr.address, port("worker"+strconv.Itoa(i)),
			MapFunc, ReduceFunc, -1, false)
	}
	mr.Wait()
...
func TestOneFailure(t *testing.T) {
	mr := setup()
	// Run 2 workers. The first worker will fail after 10 tasks
	go RunWorker(mr.address, port("worker"+strconv.Itoa(0)),
		MapFunc, ReduceFunc, 10, false)
...
func TestManyFailures(t *testing.T) {
	...
	for !done {
		...
		default:
			...
			go RunWorker(mr.address, w, MapFunc, ReduceFunc, 10, false)
			i++
			w = port("worker" + strconv.Itoa(i))
			go RunWorker(mr.address, w, MapFunc, ReduceFunc, 10, false)
			i++
